...
const SeatLayout = () => {
  ...
  const getShow = async () => {
    try {
      ...
      if (data.success) setShow({ movie: data.movie, dateTime: data.shows });
  ...
  const renderSeatIcon = (seatId, isSelected, isOccupied) => (
    <button
      key={seatId}
    ...
    >
      {!isOccupied && (
        <div className={`absolute top-0 left-0 w-full h-[40%] bg-gradient-to-b from-white/10 to-transparent pointer-events-none ${isSelected ? 'opacity-40' : 'opacity-10'}`} />
      )}
      <Armchair className={`w-5 h-5 transition-all duration-300 ${isSelected ? 'scale-110 drop-shadow-md' : 'group-hover:scale-110 group-hover:rotate-3'}`} />
      <span className={`
        absolute -bottom-1 left-1/2 -translate-x-1/2 text-[7px] font-black tracking-tighter uppercase transition-all duration-300
        ${isSelected ? 'text-black/60 opacity-100' : 'text-primary opacity-0 group-hover:opacity-100 group-hover:bottom-1'}
      `}>{seatId}</span>
  ...
  useEffect(() => {
    if (location.search.includes('success=true')) setRefresh(prev => !prev);
  }, [location]);

  useEffect(() => { getShow(); }, [refresh]);
  ...
  return show ? (
    <div className='min-h-screen bg-[#020202] text-white pt-48 pb-32 px-6 md:px-16 lg:px-24 relative overflow-hidden'>
      ...
      <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_50%_0%,rgba(var(--primary-rgb),0.03)_0%,transparent_50%)] pointer-events-none" />
      ...
      <div className='max-w-[1400px] mx-auto flex flex-col xl:flex-row gap-16 relative z-10'>
        ...
        <div className='xl:w-96 flex flex-col gap-8'>
          ...
          <div className='bg-white/[0.03] backdrop-blur-3xl border border-white/5 rounded-[2.5rem] p-8'>
            <div className='flex items-center justify-between mb-8'>
               <h3 className='text-[10px] font-black text-gray-400 uppercase tracking-widest'>Show Times</h3>
              ...
              {show?.dateTime?.[date]?.map((item) => (
                ...
                >
                  <div className='flex items-center gap-4'>
                    <Clock className={`w-5 h-5 ${selectedTime?.time === item.time ? 'text-black' : 'text-primary'}`} />
                    <span className='font-black text-lg'>{isoTimeFormat(item.time)}</span>
                  </div>
                  {selectedTime?.time === item.time && <CheckCircle2 className='w-5 h-5' />}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className='flex-1 flex flex-col items-center'>
                  ...
                  <div key={row} className='flex items-center gap-6'>
                    ...
                    <div className='flex gap-3'>
                      {Array.from({ length: 9 }, (_, i) => {
                        ...
                        return renderSeatIcon(seatId, selectedSeats.includes(seatId), occupiedSeats.includes(seatId));
                      })}
                    </div>
                  </div>
                ))}
          ...
          {/* LEGEND */}
          <div className='flex gap-10 mt-20 border-t border-white/5 pt-10'>
            <div className='flex items-center gap-3'><div className='w-4 h-4 rounded-lg bg-[#121212] border border-white/10' /><span className='text-[9px] font-black text-gray-500 uppercase tracking-widest'>Available</span></div>
            <div className='flex items-center gap-3'><div className='w-4 h-4 rounded-lg bg-primary' /><span className='text-[9px] font-black text-gray-500 uppercase tracking-widest'>Selected</span></div>
            <div className='flex items-center gap-3'><div className='w-4 h-4 rounded-lg bg-white/5 opacity-40' /><span className='text-[9px] font-black text-gray-500 uppercase tracking-widest'>Booked</span></div>
          </div>
        </div>
      </div>
        ...
        <div className='bg-white/10 backdrop-blur-3xl border border-white/10 p-5 rounded-[2.5rem] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.8)] flex flex-col md:flex-row items-center justify-between gap-6 px-10'>
          <div className='flex items-center gap-8 text-white'>
             ...
             <div>
                ...
                <p className='text-2xl font-black tracking-tighter'>₹{(selectedSeats.length * 250).toLocaleString()}</p>
             </div>
          </div>

          <button onClick={bookTickets} className='w-full md:w-auto px-12 py-5 bg-primary text-black font-black text-xs tracking-[0.2em] uppercase rounded-2xl hover:scale-105 active:scale-95 transition-all shadow-2xl flex items-center justify-center gap-3 group'>
            Confirm & Pay <ArrowRight className='w-4 h-4 transition-transform group-hover:translate-x-1' />
          </button>
        </div>
      </div>
