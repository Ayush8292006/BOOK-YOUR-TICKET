...
const MovieDetails = () => {
  ...
  const isFavorite = favoriteMovies?.some((m) => m._id === show.movie._id) || false;
  ...
  return (
    <div className="px-6 md:px-16 lg:px-40 pt-20 md:pt-32 pb-20 bg-black text-white min-h-screen">
      ...
      <div className="flex flex-col lg:flex-row gap-12 max-w-7xl mx-auto relative">
        ...
        {/* Left Column: Poster & Large Rating */}
          ...
          <div className="flex flex-col items-center lg:items-start mt-2">
             ...
             <div className="flex items-center gap-2">
                {[...Array(5)].map((_, i) => (
                  <StarIcon 
                    key={i} 
                    className={`w-5 h-5 ${i < Math.round(show.movie.vote_average / 2) ? "fill-primary text-primary" : "text-gray-600"}`} 
                  />
                ))}
                <span className="ml-2 font-bold text-lg">{show.movie.vote_average.toFixed(1)}</span>
             </div>
          </div>
        </div>

        {/* Right Column: Title, Metadata, Cast, and Buttons */}
        <div className="flex flex-col gap-6 flex-1">
          <div>
            <p className="text-primary font-medium tracking-wide mb-2">ENGLISH</p>
            <h1 className="text-5xl md:text-6xl font-bold leading-tight">{show.movie.title}</h1>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-6 text-gray-300 text-sm">
            <span className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-full border border-white/10">
              <Clock className="w-4 h-4 text-primary" /> {timeFormat(show.movie.runtime)}
            </span>
            <span className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-full border border-white/10">
              <Film className="w-4 h-4 text-primary" /> {show.movie.genres[0]?.name}
            </span>
            <span className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-full border border-white/10">
              <Calendar className="w-4 h-4 text-primary" /> {show.movie.release_date.split("-")[0]}
            </span>
          </div>
            ...
            <div className="flex gap-5 overflow-x-auto no-scrollbar pb-2">
              {show.movie.casts?.slice(0, 6).map((cast, index) => (
                <div key={index} className="flex flex-col items-center min-w-[80px]">
                  ...
                  />
                  <p className="text-[10px] text-gray-400 mt-2 text-center line-clamp-1 w-full">{cast.name}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 mt-6">
            <a href="#dateSelect" className="bg-primary hover:bg-primary/90 text-black px-10 py-4 rounded-xl font-bold transition transform active:scale-95 shadow-lg shadow-primary/20">
              Buy Tickets
            </a>
